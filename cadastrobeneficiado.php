<?php
	...
	$historia = mysqli_real_escape_string($con, $_POST["historia"]);

	$dados = array($nome, $cpf, $rg, $fone, $email, $senha, $login, $estado, $cidade, $historia);

	for($i = 0; $i < 10; $i++){

		if(empty($dados[$i])){

			header('location: Beneficiado.html');
    ...
    $sqlCPF = "SELECT * FROM beneficiado WHERE CPFB = '$cpf';";
    $queryCPF = mysqli_query($con, $sqlCPF);
    $rowsCPF = mysqli_num_rows($queryCPF);
    
    $erros[1] = $rowsCPF !== 0;
    
    $sqlLOG = "SELECT * FROM beneficiado WHERE LoginB = '$login';";
    $queryLOG = mysqli_query($con, $sqlLOG);
    $rowsLOG = mysqli_num_rows($queryLOG);
    
    $erros[2] = $rowsLOG !== 0;
    
    $sqlMAIL = "SELECT * FROM beneficiado WHERE EmailB = '$email';";
	...
	if(!in_array(true, $erros)){
		...
		$sqlbeneficiado = "INSERT INTO beneficiado VALUES (NULL, '$nome', '$cpf', '$historia', '$estado', '$fone', '$rg', md5('{$senha}'), '$email', '$cidade', '$login');";
	    $querybeneficiado = mysqli_query($con, $sqlbeneficiado);
		...
		if($querybeneficiado){
		    ...
		    print "<script>
		        ...
		        location.href = 'Beneficiado.html';
